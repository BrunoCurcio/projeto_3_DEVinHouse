...
public class FinalBattle {
	...
	public static void main(String[] args) {
		...
		while (flagClassandWeapon == 0) {
			...
			try {
				...
				System.err.println("Para escolher a classe digite números de 1 a 4 e para escolher a arma digite 1 ou 2.");
			}
		}
	...
	private static int chooseOption(int numberOfOptions, Scanner in) throws Exception {
		...
		if (option < 1 || option > numberOfOptions) {
			throw new Exception("Insira um número válido entre 1-" + numberOfOptions + ".");
		}
		return option;
	}
	...
	private static int decision(int numberOfOptions, Scanner in) {
		...
		do {
			try {
				...
				System.err.println("Para escolher, insira um número válido entre 1-" + numberOfOptions + ".");
			}
		} while (repeat);
		...
		if (player.dead()) {
			...
			case GLORY:
				if (player.getGender() == Gender.MALE) {
					...
					System.out
							.println("A glória que buscavas não será sua, e a cidade aguarda por sua próxima heroína.");
				}
				break;
			}
			System.exit(0);
